...
describe("syncStudentsToTeam", () => {
  it("adds all enrolled∩bound students to the team", async () => {
    ...
    const r = await syncStudentsToTeam({ ...env, ORG_INVITE_TOKEN: "t" } as unknown as Env, "ds-2026", fetcher);
  ...
  it("skips a course with no team", async () => {
    ...
    const r = await syncStudentsToTeam({ ...env, ORG_INVITE_TOKEN: "t" } as unknown as Env, "nt", (async () => new Response("{}")) as unknown as typeof fetch);
