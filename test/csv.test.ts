...
describe("toCsv", () => {
  it("emits header + row", () => {
    ...
    expect(lines[0]).toBe("nycu_id,nycu_name,github_id,github_login,google_email,line_name,created_at,updated_at");
  ...
  it("includes a bound LINE display name without exporting the stable subject", () => {
    const csv = toCsv([{ ...row, line_sub: "U-secret", line_name: "小明 LINE" }]);
    expect(csv).toContain("小明 LINE");
