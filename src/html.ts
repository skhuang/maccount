  ...
  btn.addEventListener('click', async function () {
    ...
    try {
      while (true) {
        ...
        if (d.skipped) { out.textContent = d.skipped === 'not-configured' ? NOTCONF : d.skipped; break; }
...
<form method="post" action="${base}/drive/share" class="form-stack" style="max-width:440px">
  ...
  const responderRows = [...formResponderEmails.values()]
    ...
    .join("\n");
  const responderEmailLines = [...formResponderEmails.values()]
    .map((r) => r.email)
    .sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }))
    .join("\n");
  const responderCopyList = formResponderEmails.size
    ? `<section class="utility-card" aria-labelledby="forms-group-copy-heading">
  <h3 id="forms-group-copy-heading">${t.forms_group_copy_heading}</h3>
  <p class="muted text-small">${t.forms_group_copy_note}</p>
  <label class="copy-list">${t.forms_group_copy_label}
    <textarea id="forms-group-email-list" readonly spellcheck="false">${h(responderEmailLines)}</textarea>
  </label>
  <button type="button" class="button button--secondary" data-copy-target="forms-group-email-list">${t.forms_group_copy_btn}</button>
</section>`
    : "";
