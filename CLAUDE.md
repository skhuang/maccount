...
- **編輯課程**：`POST /admin/courses` 是 upsert（同 `course_id` 再送即更新；`created_at` 只在新建時設）。課程後台的「課程設定」表單(owner)預填 `name/term/moodle_course_id/github_org/google_classroom_id/status(active|archived)` → 再送即改。`google_classroom_id` 選填（遷移 `0013_courses_classroom`）。**待辦（後續）**：用 Google Classroom API 把該課「已綁 Google」學生加入該 classroom（`invitations.create`，需 classroom scope）、並在 /me 露出該 classroom 的 Google Meet 連結。
...
```bash
npm install
npm test            # vitest，全部測試（目前 193 passed）
npx tsc --noEmit    # 型別檢查
