...
| `db/forms.ts` | D1：課程的 Google 問卷 `course_forms`（`id,course_id,title,url`，遷移 `0011`）— `listCourseForms` / `listFormsForCourses`(IN 查多課，給 /me) / `addCourseForm` / `removeCourseForm`(依 id+course_id 刪) |
...
- **課程 Google 問卷**：課程後台「Google 問卷」區，課程 staff 可新增/移除問卷（標題＋表單連結）。`POST /c/<id>/admin/forms/{add,remove}`（`requireCourseStaff`；add 僅收 `http(s)` 連結，非法→`?forms_msg=bad`）。學生端在 `/me` 對應課程下看到問卷清單並開啟填寫——**maccount 只存/秀連結**，登入與收 email 由該 Google 表單自身設定（「收集電子郵件／需登入」）負責，再以綁定的 Google email 對應回學號。不需 Forms API / 學生 token。
