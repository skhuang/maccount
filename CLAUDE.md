...
| `db/bindings.ts` | D1：`upsertBinding`（一 GitHub 只綁一 NYCU）/ `upsertGoogleBinding`（一 Google 只綁一 NYCU；只動 `google_*` 欄、不蓋 GitHub 綁定；refresh token 用 `COALESCE` 保留、無新 token 時不清掉）/ `getGoogleTokenRow`（讀加密 refresh token+scope，**刻意獨立**於 `getBinding`/`listBindings`，避免 token 漏進 CSV/admin）/ list / delete / `getBinding` / `getBindingByGithubId`+`getBindingByGithubLogin`+`getBindingByGoogleSub`+`getBindingByGoogleEmail`（替代登入反查）/ `upsertManualGoogleEmail`（管理者手動綁定：只寫 `google_email`+`source='manual'`、`google_sub` 留空，首次 Google 登入再認領；撞其他學號 email → `GoogleConflictError`）/ `updateBindingContact`（admin 編輯 name+google_email；**改 email 會清 `google_sub`+google token**、要求下次登入重新認領；查重擋撞號）。Google 欄位：`google_sub`(UNIQUE)+`google_email`（遷移 `0009_google`）；`google_refresh_token`(加密)+`google_scope`+`google_token_updated_at`（遷移 `0010_google_tokens`）。**`source` 綁定方式**（`nycu`/`moodle`/`manual`，遷移 `0024_bindings_source`）：建立時寫入、更新一律 `COALESCE(bindings.source, ?)` **保留最初值**（第一次建立方式為準；手動綁定被登入認領 sub 後仍為 `manual`） |
...
- `GET /auth/github/login`、`GET /auth/google/login` — **替代登入**入口（免 session，設 gstate/gostate）。共用 `/auth/{github,google}/callback`，callback 無 `nycu` 時走登入分支（`getBindingByGithubId`/`getBindingByGoogleSub` 反查 → 開 session）。`/auth/google/login` 若有設定 `GOOGLE_LOGIN_CLIENT_ID` 會使用獨立 External Google OAuth client（scope 固定 `openid email`；secret = `GOOGLE_LOGIN_CLIENT_SECRET`），否則沿用 `GOOGLE_CLIENT_ID`。`db/bindings.ts` 加了這兩個反查函式。
...
- **第三種帳號提供方式（管理者手動綁定）**：學生無 NYCU、無 Moodle 匯入 email 時，owner 在 `/admin` 的「手動綁定學生」表單輸入 `學號 + Google email`（`POST /admin/manual-bind`→`upsertManualGoogleEmail`，`source='manual'`）。學生之後用該 Google 帳號登入：`googleCallback`（`!session.nycu`）在 `getBindingByGoogleSub` 反查失敗後，**先以 email 反查手動綁定**（`getBindingByGoogleEmail`，**跳過 `isAllowedEnrollmentLoginEmail` 網域限制**——admin 已明確授權該 email）→ 認領 `google_sub`（存穩定 id）、開 session 進 `/me`；該 email 已被別的 Google 帳號認領 → `done.html?status=err&reason=google_email_taken`。找不到手動綁定才走原本的 Moodle enrollment email fallback（仍受網域限制）。與選課正交：**只解決登入**，成績仍需把該學號加進課程選課名單。
...
- `POST /api/grades/ingest` — 受信任的 OJ runner 推送成績；`Authorization: Bearer <GRADES_INGEST_TOKEN>`，body 為 `[{student_id,problem_id,verdict,score,max_score,updated_at,course_id?,repo?}]`，upsert 進 `grades`。`course_id` 選填（Phase 2 起 dsjudge 會帶；未帶則 fall back `DEFAULT_COURSE_ID`）。`repo` 選填＝該生此題的 repo（full_name 或完整 URL；`/me` 連到它，遷移 `0007_grades_repo`）。`assignment_id/assignment_type(lab|exam)/assignment_title` 選填（dsjudge provision 推送,遷移 `0008_grades_assignment`）。`problem_title` 選填＝該題的人類可讀標題（dsjudge 從 meta.yaml `title` 推來,遷移 `0028_grades_problem_title`）。**題目顯示三段 fallback `problem_title || assignment_title || problem_id`**（`html.ts` 的 examPage、儀表板 lab 平面列、owner/student scoreboard 欄位；`buildScoreboard` 逐題帶 `title`）——所以沒有 per-problem title 的題目至少顯示作業標題,而不是 slug id。COALESCE 保留：後續 grade push 沒帶 title 也不會清掉。**provision 在建好 repo 時就推一列(只有 repo+assignment,score/verdict 留 null)** → `/me` 解題前就看得到 repo。upsert 用 COALESCE 保留(provision 設 repo/assignment、judge 後補 score,互不清掉)。`/me`：**lab 平面列**(每題 repo「去解題」連結+分數)；**exam 群組成考試清單** → 連到 `GET /me/exam/<assignment_id>`(該生該場各程式題的 repo 連結+分數,只露自己的)。**只存分數+判定，其餘欄位忽略**（OJ 鐵則 2：學生只看分數+verdict，測資不外洩）。
- `POST /api/assignment-visibility` — 同一把 `GRADES_INGEST_TOKEN`，assignment 級的側表 `assignment_visibility`（PK `(course_id, assignment_id)`），**每個欄位只在 body 有帶時才寫**，所以 dsjudge 可以只推其中一項而不動其他：`hidden`（從 `/me` 整個隱藏該作業，不刪成績）、`scoreboard_visible`（開放學生匿名計分板）、`open_at`/`due_at`＝**考試期間**（ISO8601 帶時區，遷移 `0023_assignment_window`；顯式 null = 清掉該邊界）。**為什麼不放 `grades`**：window 屬於assignment 而非 (student, problem)，放每列會在改時間時要重寫整批，而且 grade upsert 的 COALESCE 永遠清不掉邊界（無限期的 window 是合法狀態）。`getAssignmentMeta()` 一次讀回三者，`/me` 的**考試清單**每一列用 `examWindowInline()` 附上截止時間（`listAssignmentWindows()` 一次撈完該生所有課程，不是每場考試一次查詢；只有開始時間的考試改顯示開始），`/me/exam/<id>` 與其計分板則用 `examWindowBanner()` 顯示（Asia/Taipei，過期標「已結束」，未設定的邊界顯示「未設定」）。**這是 dsjudge 推來的快照**：真相在 dsjudge `assignments/<id>.yaml`，dsjudge 沒推就不會變（考試中途延長 → dsjudge 的 /window 端點會自動推）。
...
- **查詢綁定（兩種:依課程 / 依 GitHub org）**：`/admin` 除了課程列表,另有「查詢綁定」區。`GET /admin/bindings`＝**全部綁定總表**(學號/姓名/github_login,與選課無關 → 綁定後還沒選課的學生也查得到)。`GET /admin/org/<org>`(org 限 effective orgs：各課 `github_org` 或 `COURSE_ORG`)＝**依 org 查**:即時抓該 org 的 GitHub 成員+待接受邀請(`listOrgMembers`/`listPendingOrgInvites`,各一次分頁呼叫,經 `fetchOrgMembership` 包裝——失敗不丟例外、退成空清單+錯誤訊息),用 `orgBindingView`(純函式)以 github_login join 綁定 → 每筆標 `已加入/待接受/未加入`,並另列「已在 org、未在 maccount 綁定」的帳號。**選填 `?course=<id>` 課程篩選**：附課程下拉選單,選後把列**縮到該課選課名單∩綁定**(`selectedCourse` 只接受 effectiveOrg 等於本 org 的課程,防跨 org 洩漏),並隱藏「未在 maccount 綁定」那段(對單課無意義)。皆 `requireStaff`。`/admin/bindings` 另有 **`source`「方式」欄**(nycu/moodle/manual/未知)；**owner**(`isAdmin`)每列可**編輯**(姓名+Google email,`POST /admin/bindings/edit`→`updateBindingContact`,改 email 會清 sub;撞號→`?b=err&reason=email_taken`)與**刪除**(`POST /admin/bindings/delete`,整列不可復原),staff 只能檢視。
...
- **邀請學生加入課程 GitHub org（三條路徑 + 自動邀請）**：學生要進 org 才能存取課程 repo。三條互補路徑：
  ...
  - **查核**：課程後台選課名單的「org 狀態」欄（課程範圍）或 `/admin/org/<org>?course=<id>`（org 頁課程篩選）即時看每人 `已加入/待接受/未加入`。`/admin/org/<org>` 不帶 `?course` 時是 **org-wide、含非本課學生**，所以邀請一律 course-scoped，不要從全域清單無差別邀全部。**只有匯入當下已綁 GitHub 的學生**能被邀；匯入時還沒綁的，等他之後綁定由路徑 1 補上。
...
```bash
npm install
npm test            # vitest，全部測試（目前 441 passed）
npx tsc --noEmit    # 型別檢查
...
- **機密不進版控**：`SESSION_SECRET`、`GITHUB_CLIENT_SECRET`、`NYCU_CLIENT_SECRET`、`GOOGLE_CLIENT_SECRET`、選填的 `GOOGLE_LOGIN_CLIENT_SECRET`、`GOOGLE_TOKEN_KEY` 只透過 `npx wrangler secret put` 設定，**絕不**寫進 `wrangler.toml`。
